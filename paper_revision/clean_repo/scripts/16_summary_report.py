...
def main():
    ...
    add("**Revision 01:** Ridge intercept corrected; existing analysis definitions retained. "
        "Interpretation remains provisional pending the other review points.")
    ...
    add()
    add("72 IDC prediction configurations (2 cohorts x 4 LOPO folds x 3 encoders x 3 regressors): "
        "24 ridge models refitted; 48 unchanged MLP/XGBoost models reused.")
    add()
    ...
    add()
    add("Results below use the original gate definitions and thresholds. "
        "Gate 2.3 is computed from raw absolute residuals in the existing implementation.")
    add()
    ...
    if gate1:
        ...
        add(f"| 2.1 Multi-model | Median rho > 0.4, >=50% | "
            f"{disc_g1['n_pass']}/{disc_g1['n_total']} {'PASS' if disc_g1['gate_pass'] else 'FAIL'} | "
            f"{val_g1['n_pass']}/{val_g1['n_total']} {'PASS' if val_g1['gate_pass'] else 'FAIL'} |")

    if gate2:
        disc_g2 = gate2["cohorts"]["discovery"]
        val_g2 = gate2["cohorts"]["validation"]
        add(f"| 2.2 Spatial structure | Moran's I p<0.01, >=50% | "
            f"{disc_g2['n_pass']}/{disc_g2['n_total']} {'PASS' if disc_g2['gate_pass'] else 'FAIL'} | "
            f"{val_g2['n_pass']}/{val_g2['n_total']} {'PASS' if val_g2['gate_pass'] else 'FAIL'} |")

    if gate3:
        disc_rho = gate3["cohorts"]["discovery"]["overall_median_rho"]
        val_rho = gate3["cohorts"]["validation"]["overall_median_rho"]
        threshold = gate3['threshold']
        add(f"| 2.3 Dual-track (raw) | Median rho > {threshold} | "
            f"{disc_rho:.3f} {'PASS' if disc_rho > threshold else 'FAIL'} | "
            f"{val_rho:.3f} {'PASS' if val_rho > threshold else 'FAIL'} |")
    ...
    pathways = [base / 'phase3' / 'pathways' / config['cohorts'][c]['name'] /
                'pathway_consistency.csv' for c in ['discovery', 'validation']]
    if all(p.exists() for p in pathways):
        merged = pd.read_csv(pathways[0]).merge(pd.read_csv(pathways[1]), on='pathway',
                                               suffixes=('_discovery', '_validation'))
        n_same = ((merged['mean_cohens_d_discovery'] * merged['mean_cohens_d_validation']) > 0).sum()
        add(f"**Shared pathways with the same mean-effect direction:** {n_same}/{len(merged)}. "
            "This descriptive sign count is not an independent replication test.")
    add()
    ...
    add()
    add("The numerical summaries above were regenerated after the ridge baseline correction. "
        "The original hard-coded conclusions are not carried forward. See "
        "`paper_revision/COMPUTATION_CORRECTION_01.md` for the old-versus-corrected comparison, "
        "manuscript impact, and remaining limitations. Biological interpretation and "
        "manuscript wording will be reviewed after the deferred issues are resolved.")
    add()
