...
def _fig3c_pathway_heatmap(config, logger):
    ...
    # Corrected residuals can produce both positive and negative pathway effects.
    vmax = np.ceil(np.abs(matrix).max() * 10) / 10  # round up to nearest 0.1
        ...
        for j, (d_col, repro_col, has_col) in enumerate([
            ...
            if not row[has_col]:
                ...
                ax.text(j, i, '**', ha='center', va='center', fontsize=6,
                        fontweight='bold', color='white' if abs(row[d_col]) > vmax * 0.6 else 'black')
