...
def panel_b_spatial(fig, config):
    ...
    gate = json.loads((Path(config['output_dir']) / 'phase2/gate2_2_spatial.json').read_text())
    samples = [s for cohort in gate['cohorts'].values() for s in cohort['samples'].values()]
    spatial_text = (f"Moran's I = {min(s['morans_i'] for s in samples):.2f}"
                    f"\u2013{max(s['morans_i'] for s in samples):.2f}  \u00b7  "
                    f"{sum(s['pass'] for s in samples)}/{len(samples)} sections pass spatial gate")
    fig.text(0.50, cbar_y - 0.04,
             spatial_text,
...
def panel_c_biology(fig):
    ...
    fig.text(callout_x, thumb_y - 0.01,
             'Recomputed gene, pathway and marker-score contrasts',
...
def panel_d_replication(fig, config):
    ...
    ax.text(box_x + box_w + 0.02, 0.73, 'compare', fontsize=4.5,
    ...
    ax.annotate('', xy=(box_x + box_w/2, 0.39),
                ...
                arrowprops=dict(arrowstyle='->', color=C_ARROW, lw=0.8),
                transform=ax.transAxes, annotation_clip=False)
    ax.text(box_x + box_w + 0.02, 0.43, 'compare', fontsize=4.5,
            color=C_ANNOT, style='italic', rotation=0, va='center',
            transform=ax.transAxes)

    # Recompute the descriptive sign count rather than retaining an old finding.
    base = Path(config['output_dir']) / 'phase3/pathways'
    left = pd.read_csv(base / config['cohorts']['discovery']['name'] / 'pathway_consistency.csv')
    right = pd.read_csv(base / config['cohorts']['validation']['name'] / 'pathway_consistency.csv')
    shared = left.merge(right, on='pathway', suffixes=('_discovery', '_validation'))
    same = ((shared['mean_cohens_d_discovery'] * shared['mean_cohens_d_validation']) > 0).sum()
    # Badge at bottom
    ax.text(box_x + box_w/2, 0.03,
            f'{same}/{len(shared)} shared pathways: same mean-effect sign',
...
def main():
    ...
    panel_d_replication(fig, config)
