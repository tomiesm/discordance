#!/usr/bin/env python3
"""
Script 20: COAD Generalization Proof-of-Concept

Single orchestration script for supplementary COAD (colorectal adenocarcinoma)
analysis. Demonstrates that the discordance framework generalizes beyond IDC.
...
Pipeline:
  ...
  8. Analysis results (A-D)
  9. Figure generation (S18)

Output:
    outputs/coad/
    outputs/figures/supplementary/figS18*.pdf

Usage:
    python scripts/20_coad_generalization.py
    python scripts/20_coad_generalization.py --skip-embeddings
    python scripts/20_coad_generalization.py --skip-figures
    python scripts/20_coad_generalization.py --gpu 0
"""
...
from src.plotting import (
    setup_style, save_figure, annotate_r, add_panel_label,
...
COAD_SAMPLES = ["TENX111", "TENX147", "TENX148", "TENX149"]
HEST_DIR = "./data/hest"
OUTPUT_DIR = "./outputs/coad"
IDC_OUTPUT_DIR = "./outputs"
FIGURE_DIR = "./outputs/figures/supplementary"
...
def run_qc_and_gene_panel(logger):
    """Load COAD samples, compute gene panel, and QC stats."""
    logger.info("\n=== Step 1: QC & Gene Panel ===")
    ...
    logger.info(f"  COAD panel (intersection): {len(panel)} genes")

    # IDC overlaps
    idc_panels = {}
    for name in ["biomarkers", "10x_janesick", "bridge"]:
        with open(f"data/v3/gene_list_{name}.json") as f:
            idc_panels[name] = set(json.load(f))

    idc_union = idc_panels["biomarkers"] | idc_panels["10x_janesick"]
    panel_set = set(panel)

    overlap_union = sorted(panel_set & idc_union)
    overlap_bridge = sorted(panel_set & idc_panels["bridge"])
    overlap_bio = sorted(panel_set & idc_panels["biomarkers"])
    overlap_jan = sorted(panel_set & idc_panels["10x_janesick"])

    logger.info(f"  Overlap with IDC union ({len(idc_union)}): {len(overlap_union)}")
    logger.info(f"  Overlap with IDC bridge (90): {len(overlap_bridge)}")
    ...
    cross_tissue = {
        "idc_union_overlap": overlap_union,
        "bridge_overlap": overlap_bridge,
        "biomarkers_overlap": overlap_bio,
        "janesick_overlap": overlap_jan,
        "coad_panel_size": len(panel),
        "idc_union_size": len(idc_union),
    }
    with open(out_dir / "cross_tissue_genes.json", "w") as f:
        json.dump(cross_tissue, f, indent=2)

    qc_summary = {
        "samples": qc_records,
        "n_samples": len(COAD_SAMPLES),
        "gene_panel_size": len(panel),
        "cross_tissue_overlap": len(overlap_union),
    }
    with open(out_dir / "qc_summary.json", "w") as f:
        json.dump(qc_summary, f, indent=2)

    logger.info(f"  Saved gene_panel.json, cross_tissue_genes.json, qc_summary.json")
    return panel
...
def run_embedding_extraction(gpu_id, logger):
    """Extract embeddings for COAD samples using 3 encoders."""
    logger.info("\n=== Step 2: Embedding Extraction ===")
    ...
    for enc_config in ENCODER_CONFIGS:
        ...
        # Check which samples need extraction
        need_extraction = []
        for sid in COAD_SAMPLES:
...
def run_loso_prediction(gene_panel, logger):
    ...
    folds = build_loso_folds(COAD_SAMPLES)
    ...
    for enc_config in ENCODER_CONFIGS:
        ...
        # Load embeddings for all samples
        embed_data = load_embeddings_for_samples(COAD_SAMPLES, enc_name, embed_dir)
...
def run_discordance_scores(logger):
    """Compute discordance scores for all COAD samples."""
    logger.info("\n=== Step 4: Discordance Scores ===")
    out_dir = Path(OUTPUT_DIR)
    pred_dir = out_dir / "predictions"
    ...
    # Aggregate per encoder (ridge only)
    config_names = []
    config_discordance = {}
...
def compute_morans_i_per_gene(gene_panel, logger):
    ...
    for sid in COAD_SAMPLES:
        ...
        logger.info(f"  {sid}: {len(available)} genes computed")
...
def compute_gene_features(gene_panel, morans_dict, logger):
    ...
    # Aggregate predictions across all folds per encoder, compute per-gene Pearson
    gene_pearsons = {g: [] for g in gene_panel}

    for enc_name in encoder_names:
        residuals, targets, spot_ids = aggregate_folds(pred_dir, enc_name, n_folds)
        predictions = targets - residuals  # Y_pred = Y_test - residuals
...
def run_pathway_enrichment(logger):
    """Compute pathway enrichment for COAD samples."""
    logger.info("\n=== Step 7: Pathway Enrichment ===")
    out_dir = Path(OUTPUT_DIR)
    pred_dir = out_dir / "predictions"
    scores_dir = out_dir / "scores"
    ...
    for sid in COAD_SAMPLES:
        ...
        # Load D_cond and partition spots
        ...
        # Align D_cond with residual spot order
        ...
        # Compute studentized residuals and pathway scores
...
def compile_analysis_results(gene_features_df, logger):
    """Compute all 4 analyses and save to analysis_results.json."""
    logger.info("\n=== Step 8: Analysis Results ===")
    out_dir = Path(OUTPUT_DIR)
    ...
    # Cross-sample consistency: for each pathway, how many samples have same sign?
    consistency_records = []
    ...
    coad_r = gene_features_df["mean_pearson"].values

    # Load IDC gene features
    ...
    results["analysis_c_predictability_distribution"] = {
        "coad": {
            "n_genes": len(coad_r),
            "mean": float(np.mean(coad_r)),
            "median": float(np.median(coad_r)),
            "q25": float(np.percentile(coad_r, 25)),
            "q75": float(np.percentile(coad_r, 75)),
        },
        "idc_discovery": {
            "n_genes": len(idc_bio),
            "mean": float(idc_bio["mean_pearson"].mean()),
            "median": float(idc_bio["mean_pearson"].median()),
            "q25": float(idc_bio["mean_pearson"].quantile(0.25)),
            "q75": float(idc_bio["mean_pearson"].quantile(0.75)),
        },
        "idc_validation": {
            "n_genes": len(idc_jan),
            "mean": float(idc_jan["mean_pearson"].mean()),
            "median": float(idc_jan["mean_pearson"].median()),
            "q25": float(idc_jan["mean_pearson"].quantile(0.25)),
            "q75": float(idc_jan["mean_pearson"].quantile(0.75)),
        },
    }
    logger.info(f"  C: COAD mean r={np.mean(coad_r):.3f}, "
                f"IDC disc={idc_bio['mean_pearson'].mean():.3f}, "
                f"IDC val={idc_jan['mean_pearson'].mean():.3f}")

    # --- Analysis D: Cross-Tissue Gene Predictability ---
    with open(out_dir / "cross_tissue_genes.json") as f:
        ct = json.load(f)

    shared_genes = ct["idc_union_overlap"]

    # COAD gene features (already have)
    coad_gf = gene_features_df.set_index("gene")

    # IDC gene features: average biomarkers + 10x_janesick for shared genes
    idc_bio_gf = idc_bio.set_index("gene")
    idc_jan_gf = idc_jan.set_index("gene")

    cross_records = []
    for gene in shared_genes:
        coad_val = coad_gf.loc[gene, "mean_pearson"] if gene in coad_gf.index else np.nan
        ...
        if not np.isnan(coad_val) and not np.isnan(idc_val):
            cross_records.append({"gene": gene, "coad_r": coad_val, "idc_r": idc_val})

    cross_df = pd.DataFrame(cross_records)

    if len(cross_df) >= 5:
        r_cross, p_cross = pearsonr(cross_df["coad_r"], cross_df["idc_r"])
        rho_cross, p_rho_cross = spearmanr(cross_df["coad_r"], cross_df["idc_r"])
    else:
        r_cross = rho_cross = p_cross = p_rho_cross = np.nan

    results["analysis_d_cross_tissue"] = {
    ...
    cross_df.to_csv(out_dir / "cross_tissue_gene_predictability.csv", index=False)
...
def generate_figures(gene_features_df, logger):
    """Generate supplementary figures S18a-c."""
    ...
    # --- S18a: Moran's I vs Predictability ---
    logger.info("  S18a: Moran's I vs Predictability scatter")
    gf = gene_features_df.dropna(subset=["morans_i", "mean_pearson"])
    ...
    # IDC trend for comparison (if available)
    try:
    ...
    save_figure(fig, fig_dir / "figS18a_coad_morans_scatter")
    plt.close(fig)

    # --- S18b: Pathway Enrichment Heatmap ---
    logger.info("  S18b: Pathway enrichment heatmap")
    pw_de = pd.read_csv(out_dir / "pathway_de.csv")
    ...
    # Filter to pathways significant in at least 1 sample, or top 15 by |mean d|
    ...
    if len(sig_pathways) < 5:
        top_pathways = mean_d.head(15).index.tolist()
    else:
    ...
    fig.tight_layout()
    save_figure(fig, fig_dir / "figS18b_coad_pathway_heatmap")
    plt.close(fig)

    # --- S18c: Predictability Distribution Comparison ---
    logger.info("  S18c: Predictability distribution comparison")
    idc_bio = pd.read_csv(idc_dir / "phase3" / "gene_predictability" / "biomarkers" / "gene_features.csv")
    idc_jan = pd.read_csv(idc_dir / "phase3" / "gene_predictability" / "10x_janesick" / "gene_features.csv")
    ...
    data = [idc_bio["mean_pearson"].values, idc_jan["mean_pearson"].values,
            gene_features_df["mean_pearson"].values]
    labels = ["IDC Discovery\n(280 genes)", "IDC Validation\n(280 genes)", "COAD\n(351 genes)"]
    colors = [COLORS["discovery"], COLORS["validation"], COLOR_COAD]
    ...
    save_figure(fig, fig_dir / "figS18c_coad_predictability_comparison")
    plt.close(fig)
...
def main():
    global HEST_DIR, OUTPUT_DIR, IDC_OUTPUT_DIR, FIGURE_DIR, HALLMARK_GMT
    parser = argparse.ArgumentParser(description="COAD generalization proof-of-concept")
    ...
    # Share the revision's input/output locations without changing analysis choices.
    with open(args.config) as handle:
        config = yaml.safe_load(handle)
    HEST_DIR = config["hest_dir"]
    IDC_OUTPUT_DIR = config["output_dir"]
    OUTPUT_DIR = str(Path(IDC_OUTPUT_DIR) / "coad")
    FIGURE_DIR = str(Path(IDC_OUTPUT_DIR) / "figures" / "supplementary")
    HALLMARK_GMT = config["phase3"]["hallmark_gmt"]
    ...
    logger = setup_logging(
        log_level="INFO",
        log_file=str(log_dir / "20_coad_generalization.log"),
    )

    logger.info("=" * 60)
    logger.info("COAD Generalization Proof-of-Concept")
    logger.info("=" * 60)
    logger.info(f"Samples: {COAD_SAMPLES}")
