...
class FixedAlphaRidgeRegressor(Regressor):
    ...
    PCA centers image features; the intercept restores the expression baseline
    learned exclusively from the training patients. Expression targets remain
    on their original log1p scale for prediction and residual calculation.
    """
    ...
    def fit(self, X: np.ndarray, Y: np.ndarray,
        ...
        self.model = Ridge(alpha=alpha, fit_intercept=True, solver='lsqr')
