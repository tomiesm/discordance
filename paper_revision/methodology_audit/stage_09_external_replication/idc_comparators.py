...
sys.path.insert(0,str(AUDIT/'stage_02_patient_calibration'));sys.path.insert(0,str(AUDIT/'stage_07_gene_audit'))
from calibration import load_cohort
from gene_audit import overlap_weights


def main():
    coverage=pd.read_csv(AUDIT/'stage_05_biology/coverage.csv');rows=[];members=[];checks=[]
    ...
    for cohort in ['biomarkers','10x_janesick']:
        y,r,loc,genes=load_cohort(cohort);sr=r.mean(axis=0);ae=np.abs(r).mean(axis=0);del r
        stored_locations=pd.read_csv(AUDIT/'stage_05_biology/arrays'/cohort/'locations.csv')
        ...
        for row in coverage[(coverage.cohort==cohort)&coverage.eligible].itertuples():
            path=AUDIT/'stage_05_biology/arrays'/cohort/f'{row.pathway}.npz';d=np.load(path)
            ...
            for family,panel in extpanels.items():
                ...
                for sample,ss in loc.groupby('sample_id',sort=False):
                    ix=ss.index.to_numpy();score=d['program_excluded_conditional'][ix]
                    ...
                    vals=outcomes[ix];sd=np.sqrt(((lo.sum()-1)*vals[lo].var(axis=0,ddof=1)+(hi.sum()-1)*vals[hi].var(axis=0,ddof=1))/(lo.sum()+hi.sum()-2))
                    for adjustment,a,b in [('unadjusted',lo.astype(float),hi.astype(float)),('overlap_adjusted',w1,w4)]:
                        ...
                        for k,outcome in enumerate(['observed','signed','absolute']):rows.append(dict(family=family,idc_cohort=cohort,sample=sample,patient=ss.patient.iloc[0],pathway=row.pathway,
                            n_common=len(common),outcome=outcome,adjustment=adjustment,effect=float(effect[k]),standardized_effect=float(effect[k]/sd[k]) if sd[k]>0 else np.nan,Q1_retained=ret1,Q4_retained=ret4))
        print(cohort,'common-member IDC comparisons complete',flush=True)
    df=pd.DataFrame(rows);df.to_csv(OUT/'idc_common_member_section_effects.csv',index=False)
