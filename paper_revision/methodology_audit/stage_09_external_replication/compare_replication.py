...
def main():
    ...
    old=pd.read_csv(AUDIT/'stage_06_inference/cohort_patient_summary.csv')
    old=old[old.metric.eq('standardized_estimate')]
    rows=[]; checks=[]; tailrows=[]; utility=[]; generows=[]; genecomparisons=[]; quartile_checks=[]
    iq=pd.read_csv(AUDIT/'stage_07_gene_audit/patient_prediction_quality.csv')
    ...
    for family in ['coad','idc_visium']:
        ...
        primary=es[es.grouping.eq('program_excluded') & es['tail'].eq(.25)]
            ...
            for mode in ['same_name','exact_common_members']:
                ...
                for r in ref.itertuples():
                    ...
                    for e in found.itertuples():
                        ...
                        rows.append(dict(family=family,idc_cohort=cc,comparison=mode,pathway=r.pathway,outcome=r.outcome,
                            adjustment=r.adjustment,scope=e.scope,n_common=len(members['common'][cc]),n_external_members=len(members['measured']),
                            idc_standardized_effect=r.idc_mean,external_standardized_effect=e.mean,
                            same_direction=bool(r.idc_mean*e.mean>0),idc_positive=idcpos,idc_negative=idcneg,
                            external_positive=e.n_positive,external_negative=e.n_negative,n_external_groups=e.n_groups,
                            external_ci_low=e.ci_low,external_ci_high=e.ci_high,external_loo_min=e.loo_min,external_loo_max=e.loo_max))
        # Threshold agreement is a within-analysis sensitivity, not replication.
        ...
        for key, sub in z.groupby(['specimen_group','pathway','outcome','adjustment']):
            values=sub.set_index('tail').standardized_effect.reindex([.2,.25,.3]).to_numpy()
            tailrows.append(dict(family=family,**dict(zip(['specimen_group','pathway','outcome','adjustment'],key)),
                effect_20=values[0],effect_25=values[1],effect_30=values[2],
                all_same_direction=bool(np.all(np.isfinite(values)) and (np.all(values>0) or np.all(values<0)))))
        ...
        for sample,ss in qq.groupby('sample'):
            ...
            quartile_checks.append(dict(family=family,sample=sample,specimen_group=ss.specimen_group.iloc[0],
                raw_MAE_increases_all_quartiles=bool(np.all(np.diff(v)>0)),Q4_raw_MAE_greater_Q1=bool(v[-1]>v[0]),
                Q1_mae=float(v[0]),Q4_mae=float(v[-1])))
        for group, gg in q.groupby('specimen_group'):
            ...
            utility.append(dict(family=family,specimen_group=group,mae=vals.mae,baseline_mae=vals.baseline_mae,
                relative_mae_gain=1-vals.mae/vals.baseline_mae,mean_gene_pearson=vals.mean_gene_pearson,
                Q1_mae=quart.loc[1,'mae'],Q1_baseline_mae=quart.loc[1,'baseline_mae'],Q1_relative_gain=1-quart.loc[1,'mae']/quart.loc[1,'baseline_mae'],
                Q4_mae=quart.loc[4,'mae'],Q4_baseline_mae=quart.loc[4,'baseline_mae'],Q4_relative_gain=1-quart.loc[4,'mae']/quart.loc[4,'baseline_mae']))
        ...
        for scope in (['all','excluding_P07'] if family=='idc_visium' else ['all']):
            ...
            for cc in ['biomarkers','10x_janesick']:
                ...
                merged['family']=family; merged['idc_cohort']=cc; merged['scope']=scope
                genecomparisons.append(merged)
                generows.append(dict(family=family,idc_cohort=cc,scope=scope,n_genes=len(merged),
                    spearman=float(spearmanr(merged.idc_pearson,merged.external_pearson).statistic),
                    pearson=float(pearsonr(merged.idc_pearson,merged.external_pearson).statistic)))
    df=pd.DataFrame(rows);df.to_csv(OUT/'program_comparisons.csv',index=False)
    summaries=[]
    for key,z in df.groupby(['family','idc_cohort','comparison','outcome','adjustment','scope']):
        a=z.idc_standardized_effect; b=z.external_standardized_effect
        summaries.append(dict(zip(['family','idc_cohort','comparison','outcome','adjustment','scope'],key))|
            dict(n_programs=len(z),n_same_direction=int(z.same_direction.sum()),
                n_both_positive=int(((a>0)&(b>0)).sum()),n_both_negative=int(((a<0)&(b<0)).sum()),
                spearman=float(spearmanr(a,b).statistic) if len(z)>2 else np.nan))
