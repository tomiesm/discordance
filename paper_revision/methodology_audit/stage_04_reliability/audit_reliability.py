...
def compare(a,b):
    qa=a>=np.quantile(a,.75);qb=b>=np.quantile(b,.75)
    return {'spearman':float(spearmanr(a,b).statistic),
            'Q4_overlap_fraction':float(np.sum(qa&qb)/np.sum(qa)),
            'Q4_jaccard':float(np.sum(qa&qb)/np.sum(qa|qb))}
...
def main():
    ...
    scores=pd.read_parquet(OUT.parent/'stage_03_score/spot_score_diagnostics.parquet').set_index('spot_id')
    checks,rows,spatial_rows,graph_rows,partitions=[],[],[],[],{}
    for ck,cc in cfg['cohorts'].items():
        cohort=cc['name'];base=REPO/'outputs/predictions'/cohort
        ids=[];ys=[]
        for fold in range(4):
            fd=base/'uni/ridge'/f'fold{fold}'
            ids+=json.loads((fd/'test_spot_ids.json').read_text());ys.append(np.load(fd/'test_targets.npy'))
        y=np.concatenate(ys);sc=scores.loc[ids]
        ...
        n=y.shape[1];perm=np.random.RandomState(42).permutation(n);ga,gb=perm[:n//2],perm[n//2:]
        ridge=[]
        for ec in cfg['encoders']:
            for rc in cfg['regressors']:
                ...
                a=np.abs(residual[:,ga]).mean(axis=1);b=np.abs(residual[:,gb]).mean(axis=1)
                ...
                if rc['name']=='ridge':ridge.append(np.abs(residual).astype(float))
                del residual
        mean_abs=np.mean(ridge,axis=0);del ridge
        baseline=[]
        for fold in range(4):
        ...
        np.testing.assert_allclose(baseline.mean(axis=1),sc.baseline_raw,atol=2e-7,rtol=0)
        total_full=y.sum(axis=1)
        for seed in range(42,62):
            ix=np.random.RandomState(seed).permutation(n);aidx,bidx=ix[:n//2],ix[n//2:]
            partitions[f'{cohort}/{seed}']={'A':[genes[i] for i in aidx],'B':[genes[i] for i in bidx]}
            sa=y[:,aidx].sum(axis=1);sb=y[:,bidx].sum(axis=1)
            for source,errors in [('ridge_encoder_mean',mean_abs),('median_baseline',baseline)]:
                a=errors[:,aidx].mean(axis=1);b=errors[:,bidx].mean(axis=1)
                versions={'raw':(a,b),'conditional_own_half':(compute_conditional_discordance(a,sa),compute_conditional_discordance(b,sb)),
                    'conditional_shared_full':(compute_conditional_discordance(a,total_full),compute_conditional_discordance(b,total_full))}
                for mode,(va,vb) in versions.items():
                    for sid,idx in indices.items():
                        rows.append({'cohort':cohort,'patient':sc.iloc[idx[0]].patient,'sample':sid,'seed':seed,'source':source,
                            'mode':mode,**compare(va[idx],vb[idx])})
        ...
        for sid,idx in indices.items():
            ...
            xy=df[['x','y']].to_numpy();coords=xy*cov.loc[sid,'pixel_size_um']
            w=build_spatial_weights(xy,n_neighbors=6)
            wr,wc=w.nonzero();distance=np.linalg.norm(coords[wr]-coords[wc],axis=1)
            ...
            sums=np.asarray(radius.sum(axis=1)).ravel();radius=sparse.diags(1/np.maximum(sums,1))@radius
            identity={'cohort':cohort,'patient':v.patient.iloc[0],'sample':sid}
            graph_rows.append({**identity,'n_spots':len(v),'k6_edge_median_um':np.median(distance),'k6_edge_max_um':distance.max(),
                'k6_edge_fraction_gt_150um':np.mean(distance>150),'k6_connected_components':connected_components(w,directed=False,return_labels=False),
                'radius_connected_components':connected_components(radius,directed=False,return_labels=False),
                'radius_isolated_spots':int((sums==0).sum())})
            for graph,weights in [('original_k6',w),('radius_150um',radius)]:
                for metric in ['raw','conditional','baseline_conditional','section_centered_diagnostic','total_expr','panel_genes_detected']:
                    value=v[metric].to_numpy(dtype=np.float64);obs=morans_i(value,weights)
                    z=value-value.mean();coo=weights.tocoo()
