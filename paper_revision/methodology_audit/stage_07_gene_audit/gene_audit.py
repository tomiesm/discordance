...
from calibration import load_cohort, REPO
sys.path.insert(0,str(REPO))
from src.discordance import compute_conditional_discordance
from src.pathways import load_gene_sets

MARKERS={
 'epithelial':'EPCAM KRT8 KRT18 KRT19 KRT17 CDH1 MUC1 SCUBE2 ESR1 FOXA1 TP63 COL17A1 DSP CLDN4 GATA3'.split(),
 'canonical_EMT':'VIM CDH2 FN1 SNAI1 SNAI2 ZEB1 ZEB2'.split(),
 'ECM_stromal':'HSPG2 FBLN1 COL4A1 LAMB1 MMP2 PDGFRB TIMP1 COL1A1 COL3A1 DCN FAP ACTA2 LUM'.split(),
 'macrophage':'CD163 CD68 CSF1R CD14 ITGAM'.split(),
 'proliferation':'MKI67 CENPF PCLAF'.split()}


def tails(s):
    a,b=np.quantile(s,[.25,.75]);return s<=a,s>=b
...
def overlap_weights(counts,detect,lo,hi):
    ...
    n1=np.bincount(strata[lo],minlength=25);n4=np.bincount(strata[hi],minlength=25)
    ok=(n1>=10)&(n4>=10)
...
def pearson_cols(y,p):
    a=y-y.mean(axis=0);b=p-p.mean(axis=0)
...
def summarize_patients(df):
    ...
    for key,g in patient.groupby([k for k in keys if k!='patient']):
        for metric in ['effect','standardized_effect']:
            v=g[metric].dropna().to_numpy();n=len(v)
            if not n:continue
            mean=v.mean();se=v.std(ddof=1)/np.sqrt(n) if n>1 else np.nan
            nonzero=v[v!=0];p=binomtest(int((nonzero>0).sum()),len(nonzero),.5).pvalue if len(nonzero) else 1.
            rows.append(dict(zip([k for k in keys if k!='patient'],key))|dict(metric=metric,n_patients=n,mean=mean,
                ci_low=mean-t.ppf(.975,n-1)*se,ci_high=mean+t.ppf(.975,n-1)*se,
                n_positive=int((v>0).sum()),n_negative=int((v<0).sum()),sign_p=p,
                loo_min=float(((v.sum()-v)/(n-1)).min()) if n>1 else np.nan,
                loo_max=float(((v.sum()-v)/(n-1)).max()) if n>1 else np.nan))
    cohort=pd.DataFrame(rows);cohort['sign_fdr']=np.nan
...
def main():
    ...
    contrasts=[];predictive=[];maps=[];coverage=[];checks=[];profiles=[]
    ...
    for cohort in ['biomarkers','10x_janesick']:
        ...
        y=y32.astype(float);signed=r.mean(axis=0);absolute=np.abs(r).mean(axis=0)
        pred=y-signed
        counts=np.rint(np.expm1(y));assert np.max(np.abs(counts-np.expm1(y)))<.1
        total_y=y.sum(axis=1);total_abs=absolute.sum(axis=1);total_counts=counts.sum(axis=1);det=(y>0).sum(axis=1)
        baseline=np.empty_like(y)
        for patient,g in loc.groupby('patient',sort=False):
            ix=g.index.to_numpy();train=loc.patient.ne(patient).to_numpy()
            baseline[ix]=np.abs(y[ix]-np.median(y[train],axis=0))
        sec={}
        for sample,g in loc.groupby('sample_id',sort=False):
            ix=g.index.to_numpy();ss=loc.iloc[ix].conditional.to_numpy()
            qedges=np.quantile(ss,[.25,.5,.75]);q=np.select([ss<=qedges[0],ss<=qedges[1],ss<qedges[2]],[1,2,3],default=4)
        ...
        for cat,members in MARKERS.items():
            for gene in members:coverage.append(dict(cohort=cohort,category=cat,gene=gene,measured=gene in genes))
        for j,gene in enumerate(genes):
            ...
            if j in [0,len(genes)//2,len(genes)-1]:
                ...
                err=float(np.max(np.abs(omitted-direct)));assert err<1e-10
            ...
            for sample,(ix,full,q,corr) in sec.items():
                ...
                low,high=tails(full);elo,ehi=tails(omitted[ix])
                ...
                for k in range(1,5):
                    mask=q==k
                    profiles.append(dict(**ident,quartile=k,n_spots=int(mask.sum()),**{name:float(values[mask,c].mean()) for c,name in enumerate(names)},
                        baseline_mae=float(baseline[ix,j][mask].mean())))
                for grouping,lo,hi in [('full',low,high),('gene_excluded',elo,ehi)]:
                    ...
                    for adjustment,a,b,retained1,retained4 in [('unadjusted',lo.astype(float),hi.astype(float),1.,1.),('overlap_adjusted',w1,w4,ret1,ret4)]:
                        ...
                        for c,name in enumerate(names):
                            contrasts.append(dict(**ident,grouping=grouping,adjustment=adjustment,outcome=name,
                                Q1_mean=means1[c],Q4_mean=means4[c],effect=effect[c],
                                standardized_effect=effect[c]/sd[c] if sd[c]>0 else np.nan,
                                Q1_retained=retained1,Q4_retained=retained4,n_overlap_strata=nstrata,
                                count_log2_ratio_pseudocount1=float(np.log2((means4[1]+1)/(means1[1]+1))) if name=='observed_counts' else np.nan))
                    for label,mask in [('Q1',lo),('Q4',hi)]:
                        mae=float(absolute[ix,j][mask].mean());bm=float(baseline[ix,j][mask].mean())
                        predictive.append(dict(**ident,grouping=grouping,quartile=label,n_spots=int(mask.sum()),
                            mae=mae,baseline_mae=bm,relative_mae_gain=1-mae/bm if bm>0 else np.nan,
                            signed_bias=float(signed[ix,j][mask].mean()),whole_section_gene_pearson=float(corr[j]),
                            ensemble_quartile_spearman=rho(pred[ix,j][mask],y[ix,j][mask]),
                            whole_section_mean_log=float(y[ix,j].mean()),whole_section_detection=float((y[ix,j]>0).mean()),whole_section_sd=float(y[ix,j].std())))
            if (j+1)%40==0:print(cohort,j+1,'genes complete',flush=True)
    ...
    for name,rows in frames.items():pd.DataFrame(rows).to_csv(OUT/f'{name}.csv',index=False)
