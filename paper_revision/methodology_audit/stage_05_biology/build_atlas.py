...
def contrast(y,lo,hi):
    a=np.asarray(y[lo],float);b=np.asarray(y[hi],float)
    delta=b.mean()-a.mean()
    sd=np.sqrt(((len(a)-1)*a.var(ddof=1)+(len(b)-1)*b.var(ddof=1))/(len(a)+len(b)-2))
    return {'Q1_mean':float(a.mean()),'Q4_mean':float(b.mean()),'delta':float(delta),
            'cohens_d':float(delta/sd) if sd else np.nan,'pooled_sd':float(sd),'n_Q1':len(a),'n_Q4':len(b)}


def tails(s,t=.25):
    a,b=np.quantile(s,[t,1-t]);return s<=a,s>=b
...
def main():
    ...
    cov=pd.read_csv(OUT.parent/'stage_01_provenance/coverage_summary.csv').set_index('sample')
    diagnostics=pd.read_parquet(OUT.parent/'stage_03_score/spot_score_diagnostics.parquet').set_index('spot_id')
    coverage,contrasts,quartiles,continuous,overlaps,gene_rows,scale_rows,checks,predictive=[],[],[],[],[],[],[],[],[]
    for cc in cfg['cohorts'].values():
        cohort=cc['name'];genes=panels[cohort];gmap={g:i for i,g in enumerate(genes)}
        dest=OUT/'arrays'/cohort;dest.mkdir(parents=True,exist_ok=True)
        yfold=[];ids=[];sr=[];ae=[]
        for fold in range(4):
            fd=REPO/'outputs/predictions'/cohort/'uni/ridge'/f'fold{fold}'
            yfold.append(np.load(fd/'test_targets.npy'));ids+=json.loads((fd/'test_spot_ids.json').read_text())
            r=np.stack([np.load(REPO/'outputs/predictions'/cohort/ec['name']/'ridge'/f'fold{fold}'/'test_residuals.npy') for ec in cfg['encoders']])
            sr.append(r.mean(axis=0));ae.append(np.abs(r).mean(axis=0));del r
        y=np.concatenate(yfold);signed=np.concatenate(sr);absolute=np.concatenate(ae);del sr,ae
        baseline=[]
        for fold in range(4):
        ...
        baseline=np.concatenate(baseline);del yfold
        ...
        sc['x_um']=coords.x*sc.sample_id.map(cov.pixel_size_um);sc['y_um']=coords.y*sc.sample_id.map(cov.pixel_size_um)
        ...
        for sid,ix in sample_idx.items():
            ...
            for j,gene in enumerate(genes):
                r=signed[ix,j];z=legacy[ix,j];mask=r!=0
                scales=np.divide(r[mask],z[mask])
                b=np.searchsorted(np.percentile(y[ix,j],np.linspace(0,100,21)[1:-1]),y[ix,j],side='right')
                ns=np.bincount(b,minlength=20);occupied=ns[ns>0]
                scale_rows.append({'cohort':cohort,'sample':sid,'patient':sc.iloc[ix[0]].patient,'gene':gene,
                    'min_scale':float(scales.min()) if len(scales) else np.nan,
                    'median_scale':float(np.median(scales)) if len(scales) else np.nan,
                    'max_weight':float(np.max(1/scales)) if len(scales) else np.nan,
                    'mean_abs_unscaled':float(np.abs(r).mean()),'mean_abs_legacy_scaled':float(np.abs(z).mean()),
                    'occupied_bins':len(occupied),'small_bins_lt3':int((occupied<3).sum()),
                    'max_abs_legacy_residual':float(np.abs(z).max())})
            ...
            for j,gene in enumerate(genes):
                ...
                a=counts[ix,j][lo].mean();b=counts[ix,j][hi].mean()
                row.update({'Q1_mean_counts':float(a),'Q4_mean_counts':float(b),'log2_count_mean_ratio_pseudocount1':float(np.log2((b+1)/(a+1))),
                    'legacy_log2_mean_logexpression_ratio':float(np.log2((y[ix,j][hi].mean()+1e-6)/(y[ix,j][lo].mean()+1e-6))),
                    'Q1_detected_fraction':float((y[ix,j][lo]>0).mean()),'Q4_detected_fraction':float((y[ix,j][hi]>0).mean())})
                gene_rows.append(row)
        for pname,pgenes in sets.items():
            members=[g for g in pgenes if g in gmap];shared=[g for g in members if g in common]
            coverage.append({'cohort':cohort,'pathway':pname,'n_genes':len(members),'n_total':len(pgenes),'fraction_covered':len(members)/len(pgenes),
                'eligible':len(members)>=5,'genes':';'.join(members),'n_common':len(shared),'common_eligible':len(shared)>=5,'common_genes':';'.join(shared)})
            if len(members)<5:continue
            pi=np.array([gmap[g] for g in members]);other=np.array([i for i in range(len(genes)) if i not in set(pi)])
            assert not set(pi)&set(other) and len(pi)+len(other)==len(genes)
            outside_sum=y[:,other].sum(axis=1);outside_count=counts[:,other].sum(axis=1);outside_detect=(y[:,other]>0).sum(axis=1)
            raw=absolute[:,other].mean(axis=1);cond=compute_conditional_discordance(raw,outside_sum)
            section_cond=np.empty(len(y),float)
            for ix in sample_idx.values():section_cond[ix]=compute_conditional_discordance(raw[ix],outside_sum[ix])
            outcomes={'observed':y[:,pi].mean(axis=1),'signed':signed[:,pi].mean(axis=1),'absolute':absolute[:,pi].mean(axis=1),
                      'legacy_scaled_signed':legacy[:,pi].mean(axis=1)}
            if len(shared)>=5:
            ...
            np.savez_compressed(dest/f'{pname}.npz',**{k:np.asarray(v) for k,v in outcomes.items()},**variants,
                outside_sum_log_expression=outside_sum,outside_total_counts=outside_count,outside_detected_genes=outside_detect,
                program_baseline_mae=pbase)
            for sid,ix in sample_idx.items():
                ...
                checks.append({'check':f'{sid}/{pname}:legacy_score_reproduction','max_abs':diff,'pass':diff<2e-5})
                assert diff<2e-5
                for method,values in variants.items():
                    vals=values[ix]
                    lo,hi=tails(vals)
                    overlaps.append({**identity,'grouping':method,'Q1_overlap_fraction':float(np.sum(lo&original_lo)/np.sum(original_lo)),
                        'Q4_overlap_fraction':float(np.sum(hi&original_hi)/np.sum(original_hi)),
                        'rank_correlation':float(spearmanr(vals,variants['full_conditional'][ix]).statistic)})
                    tailset=[.2,.25,.3] if method=='program_excluded_conditional' else [.25]
                    for tail in tailset:
                    ...
                    qedges=np.quantile(vals,[.25,.5,.75])
                    ...
                    for qname,mask in [('Q1',lo),('Q4',hi)]:
                        mae=outcomes['absolute'][ix][mask].mean();bm=pbase[ix][mask].mean()
    ...
    for name,rows in [('coverage',coverage),('section_contrasts',contrasts),('quartile_profiles',quartiles),('continuous_associations',continuous),
        ('group_membership_sensitivity',overlaps),('gene_characterization',gene_rows),('legacy_scaling_diagnostics',scale_rows),('program_prediction_quality',predictive)]:
