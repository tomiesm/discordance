...
def null_moments(a, groups, y):
    ...
    node_p = p[labels]
    expected_pairs = .5*(node_p@(a@node_p) - node_p@node_p)
    internal_degree = np.asarray(counts[np.arange(n), labels]).ravel()-1
    internal_pairs = .5*np.bincount(labels, weights=internal_degree, minlength=len(groups))
    pair_probability = np.divide(positives*(positives-1), sizes*(sizes-1),
                                 out=np.zeros(len(groups)), where=sizes > 1)
...
def summarize_stats(a, y, totals, mean, variance):
    ...
    if y.ndim == 1:
        y = y[:, None]
        count = count[:, None]
    pairs = .5*np.sum(y*(count-y), axis=0)
    qualifying = ((totals[:, None] >= 20) & (count >= 5) & (count/totals[:, None] >= .1)).sum(axis=0)
...
def main():
    ...
    if not frozen.exists():
        frozen.mkdir()
        for file in ['protocol.json', 'spatial_test.py']:
            shutil.copy2(HERE/file, frozen/file)
        inputs = {}
        for sample in PROTOCOL['samples']:
            for name in ['cell_evidence.parquet', 'measured_cells.h5ad']:
                p = PREVIOUS/f'results/{sample}/{name}'
                inputs[str(p)] = sha(p)
        dump(HERE/'input_manifest.json', {'utc': datetime.now(timezone.utc).isoformat(), 'files': inputs,
             'protocol_sha256': sha(HERE/'protocol.json')})
        dump(frozen/'manifest.json', {p.name: sha(p) for p in frozen.iterdir() if p.is_file()})
    rows = []
    component_rows = []
    balance_rows = []
    configurations = []
    for sample_index, (sample, patient) in enumerate(PROTOCOL['samples'].items()):
        ...
        for null, groups in groups_by_null.items():
            ...
            stratum_summary.append(dict(null=null, n_strata=len(groups), minimum_size=min(map(len, groups)),
                median_size=float(np.median(list(map(len, groups)))), maximum_size=max(map(len, groups))))
        obs.to_parquet(OUT/f'{sample}_tumor_inputs.parquet')
        ...
        covariates = np.column_stack([np.log1p(obs.nonmarker_transcripts), np.log1p(obs.cell_area),
                                     obs.nonmarker_detected_genes, obs.local_stromal_fraction, obs.local_myoepithelial_fraction])
        for radius in PROTOCOL['radii_um']:
            ...
            for null_index, (null, groups) in enumerate(groups_by_null.items()):
                for mark in (['coexpression', 'nuclear'] if radius == 100 and null == 'technical' else ['coexpression']):
                    ...
                    configuration = dict(sample=sample, patient=patient, radius_um=radius, null=null, mark=mark,
                        n_cells=len(obs), n_positive=int(y.sum()), n_strata=len(groups),
                        n_positive_in_frozen_strata=int(positive[positive == sizes].sum()),
                        expected_pairs_analytic=expected_pairs)
                    ...
                    for j, name in enumerate(['positive_pairs', 'qualifying_centers', 'maximum_local_z']):
                        values = null_stats[:, j]
                        row = dict(**configuration, statistic=name, observed=float(observed[j]),
                            null_mean=float(values.mean()), null_sd=float(values.std(ddof=1)),
                            null_025=float(np.quantile(values, .025)), null_975=float(np.quantile(values, .975)),
                            observed_over_null_mean=float(observed[j]/values.mean()) if values.mean() > 0 else None,
                            p_permutation=pvalue(values, observed[j], 1e-10 if j == 2 else 0))
                        rows.append(row)
                    if radius == 100:
                        for j, name in enumerate(covariate_names):
                            balance_rows.append(dict(**configuration, covariate=name,
                                observed_positive_mean=float(y@covariates[:, j]/y.sum()),
                                null_mean=float(balance[:, j].mean()), null_025=float(np.quantile(balance[:, j], .025)),
                                null_975=float(np.quantile(balance[:, j], .975))))
                    if radius == 100 and mark == 'coexpression':
                        ...
                        eligible = (totals >= 20) & (variance > 1e-12)
                        pscan[~eligible] = 1
                        local = pd.DataFrame({'cell_id': obs.index, 'x_um': coords[:, 0], 'y_um': coords[:, 1],
                            'n_tumor_neighbors': totals.astype(int), 'observed_candidates': counts[:, 0].astype(int),
                            'null_expected_candidates': mean, 'null_variance': variance, 'z': z,
                            'p_scan_section': pscan, 'p_scan_three_sections': np.minimum(1, 3*pscan),
                            'old_component': obs.candidate_component_100um.to_numpy(), 'eligible': eligible})
                        ...
                        for component in sorted(set(local.old_component)-{-1}):
                            ...
                            component_rows.append(dict(sample=sample, patient=patient, null=null, component=int(component),
                                n_original_centers=len(subset), max_z=float(best.z), p_scan_section=float(best.p_scan_section),
                                p_scan_three_sections=float(best.p_scan_three_sections),
                                n_centers_scan_p_le_005=int((subset.p_scan_three_sections <= .05).sum()),
                                best_center_cell_id=str(best.cell_id), x_um=float(best.x_um), y_um=float(best.y_um)))
    ...
    dump(OUT/'ANALYSIS_COMPLETE.json', {'status': 'complete', 'utc': datetime.now(timezone.utc).isoformat(),
        'configurations': len(configurations), 'permutations_per_configuration': PROTOCOL['permutations'],
        'primary_rows': table.loc[primary].dropna(axis=1).to_dict('records')})
