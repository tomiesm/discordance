...
BUILD=Path(__file__).resolve().parent;OUT=BUILD.parent;REV=OUT.parent;ROOT=REV.parent
A=REV/'methodology_audit';T=A/'stage_15_donor_correction/tables';R=REV/'clean_repo';OLD=REV/'CIBM_submission';SRC=OUT/'source'
for p in [SRC,OUT/'previews',OUT/'source_data']:p.mkdir(exist_ok=True,parents=True)
...
def oldtable(name):return pd.read_csv(track(A/'stage_13_figure_package/tables'/(name+'.csv')))
...
@lru_cache(None)
def locations():
    d=pd.read_parquet(track(A/'stage_13_figure_package/tables/locations.parquet'));reg=table('specimen_registry').set_index('sample').patient
...
def thumb(sample):
    ...
    pixel=audit('stage_01_provenance/coverage_summary.csv').set_index('sample').loc[sample,'pixel_size_um'];return im,scale,pixel
def limits(ax,ss):
