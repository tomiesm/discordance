...
def compare(x, y):
    a, b, _, _ = groups(x)
    c, d, _, _ = groups(y)
    return {'spearman': rho(x, y), 'Q1_overlap_fraction': float(np.sum(a & c) / np.sum(a)), 'Q4_overlap_fraction': float(np.sum(b & d) / np.sum(b)), 'Q1_jaccard': float(np.sum(a & c) / np.sum(a | c)), 'Q4_jaccard': float(np.sum(b & d) / np.sum(b | d))}
...
def main():
    ...
    base = pd.read_parquet(OUT.parent / 'prediction/spot_prediction_diagnostics.parquet').set_index('spot_id')
    coverage = pd.read_parquet(OUT.parent / 'coverage/spot_coverage.parquet')
    ...
    for cc in cfg['cohorts'].values():
        ...
        s['raw'] = s[rcols].mean(axis=1)
        s['conditional'] = s[ccols].mean(axis=1)
        ...
        for enc in cfg['encoders']:
            ...
            checks.append({'check': f"{cohort}/{enc['name']}:conditional_reconstruction", 'max_abs': err, 'pass': err < 2e-06 and np.allclose(got, ref, atol=1e-12, rtol=0)})
        ...
        for b, g in s.groupby('pooled_expression_bin'):
            for (sid, patient), h in g.groupby(['sample_id', 'patient']):
                binrows.append({'cohort': cohort, 'bin': b, 'lower_edge': edges[b], 'upper_edge': edges[b + 1], 'sample': sid, 'patient': patient, 'n': len(h), 'fraction_of_pooled_bin': len(h) / len(g), 'mean_raw': h.raw.mean(), 'mean_conditional': h.conditional.mean(), 'sd_conditional': h.conditional.std(), 'mean_baseline_conditional': h.baseline_conditional.mean()})
        ...
        for fold in range(4):
            ...
            for sid, g in f.groupby('sample_id', sort=False):
                m = f.sample_id.to_numpy() == sid
                e = error[m]
                q1, q4, _, _ = groups(g.conditional.to_numpy())
                mean = e.mean(axis=0)
                delta = e[q4].mean(axis=0) - e[q1].mean(axis=0)
                share = mean / mean.sum()
                positive = np.maximum(delta, 0)
                share_delta = positive / positive.sum()
                concentrations.append({'cohort': cohort, 'sample': sid, 'patient': g.patient.iloc[0], 'n_genes': len(genes), 'largest_gene_error_share': share.max(), 'top10_gene_error_share': np.sort(share)[-10:].sum(), 'effective_n_error_genes': 1 / np.square(share).sum(), 'top10_positive_Q4_Q1_error_difference_share': np.sort(share_delta)[-10:].sum(), 'n_genes_with_positive_error_difference': int((delta > 0).sum())})
                for j, gene in enumerate(genes):
                    genelist.append({'cohort': cohort, 'sample': sid, 'patient': g.patient.iloc[0], 'gene': gene, 'mean_abs_error': mean[j], 'mean_error_share': share[j], 'Q4_Q1_abs_error_difference': delta[j], 'positive_difference_share': share_delta[j]})
            del error
        for sid, g in s.groupby('sample_id', sort=False):
            g = g.copy()
            identity = {'cohort': cohort, 'sample': sid, 'patient': g.patient.iloc[0]}
            ...
            for c in covariates:
                a = g[c].to_numpy()
                pooled = np.sqrt((np.var(a[q1], ddof=1) + np.var(a[q4], ddof=1)) / 2)
                contrasts.append({**identity, 'covariate': c, 'Q1_mean': np.mean(a[q1]), 'Q4_mean': np.mean(a[q4]), 'Q1_median': np.median(a[q1]), 'Q4_median': np.median(a[q4]), 'Q4_Q1_standardized_difference': (np.mean(a[q4]) - np.mean(a[q1])) / pooled if pooled else np.nan})
            for tail in [0.2, 0.25, 0.3]:
                for metric in ['raw', 'conditional', 'baseline_conditional', 'section_centered_diagnostic']:
                    a, b, lo, hi = groups(g[metric].to_numpy(), tail)
                    cutoffs.append({**identity, 'score': metric, 'tail_fraction': tail, 'lower_threshold': lo, 'upper_threshold': hi, 'n_lower': int(a.sum()), 'n_upper': int(b.sum()), 'ties_lower': int((g[metric] == lo).sum()), 'ties_upper': int((g[metric] == hi).sum()), 'disjoint': not bool(np.any(a & b)), 'primary_Q1_recovered_fraction': float(np.sum(a & q1) / np.sum(q1)), 'primary_Q4_recovered_fraction': float(np.sum(b & q4) / np.sum(q4))})
            ...
            for b, h in g.groupby('section_expression_bin'):
                depths.append({**identity, 'section_bin': b, 'n': len(h), 'expression_mean': h.total_expr.mean(), 'raw_mean': h.raw.mean(), 'raw_sd': h.raw.std(), 'conditional_mean': h.conditional.mean(), 'conditional_sd': h.conditional.std(), 'Q4_fraction': h.Q4.mean(), 'panel_counts_median': h.panel_counts.median(), 'detected_genes_mean': h.panel_genes_detected.mean()})
            allspots.append(g.reset_index()[['spot_id', 'sample_id', 'patient', 'cohort', 'raw', 'conditional', 'baseline_raw', 'baseline_conditional', 'section_centered_diagnostic', 'total_expr', 'panel_counts', 'noncontrol_counts', 'panel_genes_detected', 'tissue_fraction', 'pooled_expression_bin', 'section_expression_bin']])
        print(cohort, 'completed', len(s), 'spots', flush=True)
    for name, values in [('score_covariate_correlations', correlations), ('score_comparisons', comparisons), ('section_depth_profiles', depths), ('pooled_bin_composition', binrows), ('cutoffs_and_tail_sensitivities', cutoffs), ('quartile_covariate_contrasts', contrasts), ('gene_error_contributions', genelist), ('gene_concentration', concentrations)]:
