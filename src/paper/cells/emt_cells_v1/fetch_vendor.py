...
import requests
import zipfile
HERE = cell_dir('emt_cells_v1')
ARCHIVES = {'NCBI785': ('GSM7780153', 'GSM7780153_Xenium_FFPE_Human_Breast_Cancer_Rep1_outs.zip'), 'NCBI784': ('GSM7780154', 'GSM7780154_Xenium_FFPE_Human_Breast_Cancer_Rep2_outs.zip'), 'NCBI783': ('GSM7780155', 'GSM7780155_Xenium_V1_FFPE_Preview_Human_Breast_Cancer_Sample_2_outs.zip')}
...
def main():
    parameters = json.loads((CELL_SOURCE/'emt_cells_v1/protocol.json').read_text())
    for filename in parameters['source_annotations'].values():
        gsm = filename.split('_')[0]
        target = HERE/'sources'/filename
        target.parent.mkdir(parents=True, exist_ok=True)
        if not target.exists():
            response = requests.get(f'https://ftp.ncbi.nlm.nih.gov/geo/samples/GSM7780nnn/{gsm}/suppl/{filename}', timeout=120)
    ...
    for sample, (gsm, filename) in ARCHIVES.items():
        ...
        with zipfile.ZipFile(RangeReader(url)) as archive:
            ...
            for member in archive.infolist():
                ...
                selected = basename in ['cell_feature_matrix.h5', 'cells.parquet']
                if not selected:
                    continue
                path = out / basename
                ...
                manifest.append(dict(url=url, member=member.filename, bytes=member.file_size, crc32=member.CRC, sha256=hashlib.sha256(path.read_bytes()).hexdigest()))
