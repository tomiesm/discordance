...
from src.paper.cells.emt_cells_v1.common import *
...
def source_group(label):
    if label in ['DCIS_1', 'DCIS_2', 'Invasive_Tumor', 'Prolif_Invasive_Tumor', 'Tumor']:
        return 'Tumor'
    if label in ['Stromal', 'Stromal Normal', 'Tumor Associated Stromal']:
        return 'Stromal'
    if label.startswith('Myoepi') or label == 'DST+ Myoepithelial':
        return 'Myoepithelial'
    if label in ['ESR1+ Epithelial', 'PIGR+ Epithelial', 'OPRPN+ Epithelial']:
        return 'Normal epithelial'
    if label == 'Transitional Cells':
        return 'Published transitional'
    if label in ['Unlabeled', 'Not Plotted', 'Missing annotation'] or 'Hybrid' in label:
        return 'Uncertain/hybrid'
...
def fit_coordinate_transform(native, histology):
    ...
    if projective_error <= 1:
        ...
        model = RBFInterpolator(native[::2], histology[::2], neighbors=32, kernel='thin_plate_spline', smoothing=1e-08)
        error = np.linalg.norm(model(native[1::2]) - histology[1::2], axis=1)
        if np.max(error) > 1:
            raise RuntimeError(f'Local coordinate registration discrepancy: {np.max(error)} pixels')
        model = RBFInterpolator(native, histology, neighbors=32, kernel='thin_plate_spline', smoothing=1e-08)
        info = {'method': 'local thin-plate interpolation of supplied transcript coordinates', 'neighbors': 32, 'smoothing': 1e-08}
    return (model, dict(**info, n_control_points=len(native), global_projective_max_error_pixels=projective_error, test_max_error_pixels=float(error.max()), test_p99_error_pixels=float(np.quantile(error, 0.99)), test_median_error_pixels=float(np.median(error))))
...
def main():
    ...
    for sample, patient in PROTOCOL['samples'].items():
        ...
        if (out / 'COUNTS_VERIFIED.json').exists():
            ...
            summaries.append(json.loads((out / 'COUNTS_VERIFIED.json').read_text()))
            continue
        ...
        if numeric:
            ...
            cell_ids = np.array([x.decode() if isinstance(x, bytes) else str(x) for x in cell_ids])
        ...
        counts = sparse.csr_matrix(reconstructed[:, bio])
        nuc = sparse.csr_matrix(nuclear[:, bio])
        ...
        registration = dict(method='Direct centroid of measured assigned Q>=20 biological transcripts in the supplied H&E frame; no fitted transform applied', primary_spatial_analysis='Vendor cell centroids in native micrometers', he_usage='Visualization and secondary spot assignment only; transcript centroid is not the geometric cell centroid', native_transcript_vs_cell_centroid_um_quantiles=np.nanquantile(displacement, [0.5, 0.95, 0.99, 1]).tolist())
        np.testing.assert_array_equal(coordinate_counts, total)
        obs = pd.DataFrame({'sample': sample, 'patient': patient, 'source_label': labels.to_numpy(), 'source_group': [source_group(x) for x in labels], 'x_um': native[:, 0], 'y_um': native[:, 1], 'he_x': he[:, 0], 'he_y': he[:, 1], 'total_biological_transcripts': total, 'detected_biological_genes': detected, 'cell_area': cells.cell_area.to_numpy(), 'nucleus_area': cells.nucleus_area.to_numpy()}, index=ids)
        obs['transcript_native_x_um'] = measured_centroid[:, 0]
        obs['transcript_native_y_um'] = measured_centroid[:, 1]
        ...
        data.obsm['spatial_um'] = native
        data.obsm['spatial_he'] = he
        data.write_h5ad(out / 'measured_cells.h5ad', compression='gzip')
        summary = dict(sample=sample, patient=patient, n_cells=len(ids), n_biological_genes=int(bio.sum()), transcript_rows=total_rows, high_q_rows=high_q_rows, unassigned_high_q=unassigned, unassigned_identifiers=unknown_values, vendor_matrix_exact_match=True, n_qc_pass=int(obs.qc_pass.sum()), n_source_tumor_qc=int((obs.qc_pass & (obs.source_group == 'Tumor')).sum()), n_annotated=int((obs.source_label != 'Missing annotation').sum()), labels=obs.source_label.value_counts().to_dict(), coordinate_transform=registration)
        dump(out / 'COUNTS_VERIFIED.json', summary)
