...
def summary(y, pred, mean, median):
    ...
    return {'n_spots': len(y), 'mae': float(raw.mean()), 'rmse': float(np.sqrt(mse)), 'mean_signed_error': float(r.mean()), 'mean_absolute_gene_bias': float(np.abs(r.mean(axis=0)).mean()), 'negative_prediction_fraction': float(np.mean(pred < 0)), 'baseline_median_mae': float(babs.mean()), 'baseline_mean_rmse': float(np.sqrt(bmse)), 'mae_improvement_fraction': float(1 - raw.mean() / babs.mean()), 'mse_improvement_fraction': float(1 - mse / bmse), 'spots_better_than_median_baseline_fraction': float(np.mean(raw < babs))}
...
def gene_summary(y, p, mean, median):
    ...
    return pd.DataFrame({'pearson': corr, 'mae': np.abs(r).mean(axis=0), 'rmse': np.sqrt(np.square(r).mean(axis=0)), 'bias': r.mean(axis=0), 'baseline_median_mae': np.abs(y - median).mean(axis=0), 'baseline_mean_rmse': np.sqrt(np.square(y - mean).mean(axis=0))})
...
def main():
    ...
    for cc in cfg['cohorts'].values():
        ...
        for fold, y32 in enumerate(ys):
            ...
            median = np.concatenate([np.median(train[:, j:j + 32].astype(np.float64), axis=0) for j in range(0, len(genes), 32)])
            del train
            ...
            for sid in split['test_samples']:
                ...
                checks.append({'check': f'{sid}:raw_score_reconstruction', 'pass': rawerr < 2e-06, 'max_abs': rawerr})
                assert rawerr < 2e-06
                ...
                f = pd.DataFrame({'spot_id': np.array(ids)[mask], 'sample': sid, 'patient': split['test_patient'], 'cohort': cohort, 'quartile': q, 'D_cond': score, 'ridge_encoder_mean_mae': avg_error[mask], 'ridge_ensemble_mae': ensemble_error[mask], 'baseline_median_mae': bmae[mask], 'baseline_mean_mse': bmse[mask], 'sum_log1p_expression': y[mask].sum(axis=1), 'detected_genes': (y[mask] > 0).sum(axis=1)})
                diagnostics.append(f)
                for quartile, group in f.groupby('quartile'):
                    quartiles.append({'sample': sid, 'patient': split['test_patient'], 'cohort': cohort, 'quartile': int(quartile), 'n_spots': len(group), 'mean_mae': group.ridge_encoder_mean_mae.mean(), 'baseline_median_mae': group.baseline_median_mae.mean(), 'mae_improvement_fraction': 1 - group.ridge_encoder_mean_mae.mean() / group.baseline_median_mae.mean(), 'spots_better_than_baseline_fraction': (group.ridge_encoder_mean_mae < group.baseline_median_mae).mean(), 'sum_log1p_expression_mean': group.sum_log1p_expression.mean(), 'detected_genes_mean': group.detected_genes.mean()})
    ...
    pd.DataFrame(pca_rows, columns=['cohort','patient','encoder','variance_retained','components','load_warnings']).to_csv(OUT / 'ridge_pca_variance.csv', index=False)
    ...
    (OUT / 'checks.json').write_text(json.dumps({'completed_utc': datetime.now(timezone.utc).isoformat(), 'status': 'pass' if all((c['pass'] for c in checks)) else 'failed', 'checks': checks}, indent=2) + '\n')
