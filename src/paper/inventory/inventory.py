...
def main():
    ...
    for cohort_key, cc in config['cohorts'].items():
        ...
        for f in range(cc['n_lopo_folds']):
            ...
            okay = not tr.intersection(te) and (not trp.intersection(tep)) and (tr | te == set(cc['samples'])) and (tep == {split['test_patient']}) and (trp == set(split['train_patients']))
            check(f'{name}:fold{f}:patient_partition', okay)
            fold_rows.append({'cohort': name, 'fold': f, 'test_patient': split['test_patient'], 'n_train_sections': len(tr), 'n_test_sections': len(te), 'patient_partition_pass': bool(okay)})
            encoder_ids = []
            for enc in config['encoders']:
                ...
                check(f'{name}:fold{f}:{encname}:only_test_sections', all((any((s.startswith(sid + '_') for sid in te)) for s in ids)))
                for file in ['test_targets.npy', 'test_predictions.npy', 'test_residuals.npy']:
                    arr = np.load(record(fp / file), mmap_mode='r')
                    check(f'{name}:fold{f}:{encname}:{file}:shape', arr.shape == (len(ids), len(genes)), list(arr.shape))
                    del arr
                encoder_ids.append(ids)
                for sid in te:
                    test_membership[sid, encname] = [s[len(sid) + 1:] for s in ids if s.startswith(sid + '_')]
            check(f'{name}:fold{f}:encoder_id_order', all((x == encoder_ids[0] for x in encoder_ids)))
        check(f'{name}:each_section_tested_once', set(test_visits) == set(cc['samples']) and all((v == 1 for v in test_visits.values())))
        real_panels = []
        for sid in cc['samples']:
            ...
            for enc in config['encoders']:
                ...
                with h5py.File(record(ep), 'r') as f:
                    ...
                    arr = f['embeddings']
                    check(f'{sid}:{encname}:embedding_shape', arr.shape == (len(ids), enc['embed_dim']), list(arr.shape))
                ...
                encoder_rows.append({'cohort': name, 'sample': sid, 'encoder': encname, 'embedding_rows': len(ids), 'nonfinite_embedding_rows': int((~finite).sum()), 'nan_embedding_rows': nan_rows, 'embedding_ids_absent_raw': len(set(ids) - raw_set), 'test_spots': len(test_ids), 'expected_test_spots': len(expected), 'test_id_order_matches_alignment': expected == test_ids})
            m = metadata.loc[metadata['id'] == sid]
            check(f'{sid}:metadata_unique', len(m) == 1)
            md = m.iloc[0] if len(m) == 1 else {}
            section_rows.append({'cohort': name, 'sample': sid, 'patient': sample_patient[sid], 'source_patient_label': md.get('patient', ''), 'source_dataset': md.get('dataset_title', ''), 'preservation': md.get('preservation_method', ''), 'raw_spots': len(obs), 'raw_in_tissue': len(tissue_set) if tissue_set is not None else None, 'raw_features': len(var), 'noncontrol_features': len(real), 'modeled_genes': len(genes), 'supplied_patches': len(patch_ids), 'patch_shape': 'x'.join(map(str, patch_shape[1:])), 'scored_spots': len(score_ids), 'raw_spots_without_patch': len(raw_set - set(patch_ids)), 'patch_spots_not_scored': len(set(patch_ids) - set(score_ids)), 'in_tissue_without_patch': len(tissue_set - set(patch_ids)) if tissue_set is not None else None, 'retained_fraction': len(score_ids) / len(obs)})
            print(f'{sid}: raw={len(obs)} patches={len(patch_ids)} scored={len(score_ids)}', flush=True)
        common = set.intersection(*real_panels)
        check(f'{name}:modeled_panel_equals_noncontrol_intersection', common == set(genes), {'extra_raw_genes': sorted(common - set(genes)), 'extra_modeled_genes': sorted(set(genes) - common)})
        gene_rows.append({'cohort': name, 'modeled_genes': len(genes), 'raw_noncontrol_intersection': len(common), 'raw_intersection_not_modeled': sorted(common - set(genes)), 'modeled_not_in_intersection': sorted(set(genes) - common)})
    for file, rows in [('section_inventory.csv', section_rows), ('encoder_inventory.csv', encoder_rows), ('fold_inventory.csv', fold_rows)]:
        pd.DataFrame(rows).to_csv(OUT / file, index=False)
    dump('gene_panel_inventory.json', gene_rows)
    changed = [path for path, st in inputs.items() if Path(path).stat().st_size != st['bytes'] or Path(path).stat().st_mtime_ns != st['mtime_ns']]
    check('inspected_inputs_size_mtime_unchanged_during_run', not changed, changed)
    dump('inputs.json', inputs)
    dump('checks.json', {'completed_utc': datetime.now(timezone.utc).isoformat(), 'status': 'pass' if all((c['passed'] for c in checks)) else 'issues_found', 'n_checks': len(checks), 'failed': [c for c in checks if not c['passed']], 'checks': checks})
