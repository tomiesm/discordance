...
def read(p):
    return pd.read_csv(track(A / p))

def save(d, n):
    d.to_csv(T / (n + '.csv'), index=False)
    return d

def check(name, passed):
    CHECKS.append(dict(name=name, passed=bool(passed)))
    assert passed, name
clinical = pd.read_csv(PROJECT_ROOT / 'data/discovery_specimens.csv').sort_values('sample')
...
def summaries(d, keys, values, unit='patient'):
    ...
    for key, g in units.groupby(keys, dropna=False):
        ...
        for metric in values:
            ...
            half = t.ppf(0.975, n - 1) * x.std(ddof=1) / np.sqrt(n) if n > 1 else np.nan
...
def registry():
    s = relabel(read('inventory/section_inventory.csv'))
    ...
    for cohort in ['biomarkers', '10x_janesick']:
        seen = []
        for k in range(4):
            p = track(R / 'data/v3' / f'lopo_splits_{cohort}' / f'fold_{k}.json')
            f = json.loads(p.read_text())
            tr = {donors[x] for x in f['train_samples']}
            te = {donors[x] for x in f['test_samples']}
...
def genes():
    d = relabel(read('genes/section_effects.csv'))
    save(d, 'gene_section_effects')
    keys = ['cohort', 'gene', 'grouping', 'adjustment', 'outcome']
    vals = ['effect', 'standardized_effect', 'Q1_mean', 'Q4_mean', 'Q1_retained', 'Q4_retained']
    u, c = summaries(d, keys, vals)
    save(u, 'gene_donor_effects')
    save(c, 'gene_cohort_effects')
    _, b = summaries(d, keys, vals, unit='inherited_patient')
    save(b, 'gene_split_group_sensitivity')
    p = relabel(read('genes/prediction_quality.csv'))
    save(p, 'gene_section_quality')
    numeric = [x for x in p.select_dtypes('number') if x != 'n_spots']
    q = p.groupby(['cohort', 'patient', 'gene', 'grouping', 'quartile'])[numeric].mean().reset_index()
    save(q, 'gene_donor_quality')
    ...
    bridge['same_direction'] = bridge.biomarkers * bridge['10x_janesick'] > 0
    save(bridge, 'bridge_genes')
    rows = []
    ...
    qp = relabel(read('genes/quartile_profiles.csv'))
    sec = qp.groupby(['cohort', 'patient', 'sample', 'quartile'])[['absolute', 'baseline_mae', 'observed_log', 'signed']].mean().reset_index()
    save(sec, 'section_quartile_quality')
    pa = sec.groupby(['cohort', 'patient', 'quartile'])[['absolute', 'baseline_mae', 'observed_log', 'signed']].mean().reset_index()
    pa['relative_mae_gain'] = 1 - pa.absolute / pa.baseline_mae
    save(pa, 'donor_quartile_quality')
    links = []
...
def programs():
    ...
    for family in ['biomarkers', '10x_janesick', 'coad', 'idc_visium']:
        d = relabel(read(f'cutoffs/{family}/section_program_effects.csv'))
        d = d[d['tail'].isin([0.2, 0.25, 0.3])]
        sections.append(d)
        keys = ['family', 'pathway', 'grouping', 'tail', 'adjustment', 'outcome']
        vals = ['effect', 'standardized_effect', 'lower_mean', 'upper_mean', 'lower_retained', 'upper_retained']
        u, c = summaries(d, keys, vals, unit='unit')
        units.append(u)
        cohorts.append(c)
        if family == 'biomarkers':
            _, b = summaries(d, keys, vals, unit='inherited_unit')
            splits.append(b)
    save(pd.concat(sections), 'program_section_effects')
    save(pd.concat(units), 'program_unit_effects')
    save(pd.concat(cohorts), 'program_cohort_effects')
    save(pd.concat(splits), 'program_split_group_sensitivity')
    d = relabel(read('contrasts/boundary_section_effects.csv'))
    save(d, 'boundary_section_effects')
    keys = ['family', 'kind', 'endpoint', 'grouping', 'adjustment', 'band_um', 'outcome']
    u, c = summaries(d, keys, ['effect', 'fixed_standardized_effect', 'Q1_retained', 'Q4_retained'], unit='unit')
    save(u, 'boundary_unit_effects')
    save(c, 'boundary_cohort_effects')

def predictions():
    d = relabel(read('prediction/section_model_metrics.csv'))
    save(d, 'model_section_quality')
    vals = ['mae', 'rmse', 'baseline_median_mae', 'baseline_mean_rmse', 'mean_signed_error']
    q = d.groupby(['cohort', 'patient', 'encoder', 'regressor'])[vals].mean().reset_index()
    q['relative_mae_gain'] = 1 - q.mae / q.baseline_median_mae
    save(q, 'model_donor_quality')
    rows = []
    for cc in cfg['cohorts'].values():
        cohort = cc['name']
        genes = json.loads(track(R / 'data/v3' / f'gene_list_{cohort}.json').read_text())
        for k in range(4):
            fd = R / 'outputs/predictions' / cohort / 'uni/ridge' / f'fold{k}'
            y = np.load(track(fd / 'test_targets.npy')).astype(float)
            ...
            for enc in [e['name'] for e in cfg['encoders']]:
                for reg in [e['name'] for e in cfg['regressors']]:
                    ...
                    for s, ix in mask.items():
                        a = y[ix] - y[ix].mean(axis=0)
                        b = pred[ix] - pred[ix].mean(axis=0)
    ...
    d = pd.DataFrame(rows)
    save(d, 'model_gene_section_quality')
    q = d.groupby(['cohort', 'patient', 'encoder', 'regressor', 'gene']).pearson.mean().reset_index()
    save(q, 'model_gene_donor_quality')
    q = q.groupby(['cohort', 'encoder', 'regressor', 'gene']).pearson.mean().reset_index()
    save(q, 'model_gene_cohort_quality')
    save(q.groupby(['cohort', 'encoder', 'regressor']).pearson.mean().reset_index(), 'model_cohort_correlations')

def features():
    import importlib.util
    p = track(STAGE_SOURCE / 'features/audit.py')
    spec = importlib.util.spec_from_file_location('feature_audit', p)
    ...
    influence = []
    for cohort in ['biomarkers', '10x_janesick']:
        sec = relabel(read(f'features/{cohort}/section_features.csv'))
        old = read(f'features/{cohort}/revised_features.csv')
        f, u = m.features_from_sections(sec, old)
        f['cohort'] = cohort
        allfeatures.append(f)
        for name, cols in [('mean_only', m.NUM[:1]), ('expression_pathway', m.NUM[:3]), ('numeric_with_spatial', m.NUM)]:
            x, _, names, _ = m.encode(f, f, cols, False)
            b = np.linalg.lstsq(x, f.mean_pearson, rcond=None)[0]
            out, baseline, fold, stat, di = m.cv(f, cols, False)
        ...
        for donor in sec.patient.unique():
            fi, _ = m.features_from_sections(sec, old, donor)
            x, _, names, _ = m.encode(fi, fi, m.NUM, False)
            b = np.linalg.lstsq(x, fi.mean_pearson, rcond=None)[0]
            influence.append(dict(cohort=cohort, omitted_donor=donor, spatial_coefficient=b[names.index('spatial_autocorrelation')]))
    save(pd.concat(allfeatures), 'gene_features')
    save(pd.DataFrame(metrics), 'gene_feature_metrics')
    save(pd.DataFrame(coefs), 'gene_feature_coefficients')
    save(pd.DataFrame(preds), 'gene_feature_cv_predictions')
    save(pd.DataFrame(influence), 'gene_feature_influence')

def external():
    d = relabel(read('external/idc_common_member_section_effects.csv'))
    u, c = summaries(d, ['family', 'idc_cohort', 'pathway', 'outcome', 'adjustment'], ['standardized_effect'])
    save(u, 'idc_common_member_donor_effects')
    save(c, 'idc_common_member_cohort_effects')
    z = read('external/program_comparisons.csv')
    program = pd.read_csv(T / 'program_cohort_effects.csv')
    for i, r in z.iterrows():
        if r.idc_cohort != 'biomarkers':
            continue
        if r.comparison == 'exact_common_members':
            ...
            a = program[program.family.eq(r.idc_cohort) & program.pathway.eq(r.pathway) & program.outcome.eq(r.outcome) & program.adjustment.eq(r.adjustment) & program.grouping.eq('program_excluded') & program['tail'].eq(0.25) & program.metric.eq('standardized_effect')]
        assert len(a) == 1, (i, len(a))
        a = a.iloc[0]
        z.loc[i, 'idc_standardized_effect'] = a['mean']
        z.loc[i, 'idc_positive'] = a.n_positive
        z.loc[i, 'idc_negative'] = a.n_negative
    z['same_direction'] = z.idc_standardized_effect * z.external_standardized_effect > 0
    save(z, 'external_program_comparisons')
    iq = pd.read_csv(T / 'gene_donor_quality.csv')
    iq = iq[iq.grouping.eq('full') & iq.quartile.eq('Q1')]
    g = read('external/gene_predictability_pairs.csv')
    lookup = iq.groupby(['cohort', 'gene']).whole_section_gene_pearson.mean()
    g['idc_pearson'] = [lookup.loc[r.idc_cohort, r.gene] for r in g.itertuples()]
    save(g, 'external_gene_pairs')
    rows = []
    for key, a in g.groupby(['family', 'idc_cohort', 'scope']):
        rows.append(dict(zip(['family', 'idc_cohort', 'scope'], key)) | dict(n_genes=len(a), pearson=pearsonr(a.idc_pearson, a.external_pearson).statistic, spearman=spearmanr(a.idc_pearson, a.external_pearson).statistic))
    save(pd.DataFrame(rows), 'external_gene_correlations')
if __name__ == '__main__':
    for f in [registry, genes, programs, predictions, features, external]:
        f()
        print(f.__name__, 'complete', flush=True)
