...
def run(enc):
    ...
    if (dest / 'COMPLETE.json').exists():
        return json.loads((dest / 'COMPLETE.json').read_text())
    spec = {'test_samples': SAMPLES[:2], 'train_samples': SAMPLES[2:]}
    ...
    for k, sample in enumerate(SAMPLES):
        ...
        ys[sample] = np.load(fd / 'test_targets.npy')
        ids[sample] = json.loads((fd / 'test_spot_ids.json').read_text())
        ...
        assert [g for g in m if not g.startswith('__')] == genes
        ...
        for p in [fd / 'test_targets.npy', fd / 'test_spot_ids.json', hp]:
            inputs[str(p.relative_to(ROOT))] = digest(p)
    train = spec['train_samples']
    test = spec['test_samples']
    assert not set(train) & set(test)
    x = np.concatenate([xs[s] for s in train])
    y = np.concatenate([ys[s] for s in train])
    xt = np.concatenate([xs[s] for s in test])
    yt = np.concatenate([ys[s] for s in test])
    tid = sum([ids[s] for s in test], [])
    del xs, ys
    ...
    pred = model.predict(xt).astype(np.float32)
    assert np.isfinite(pred).all()
    sel = np.linspace(0, len(genes) - 1, 17, dtype=int)
    xp = model.pca.transform(x)
    zp = model.pca.transform(xt)
    ymean = y[:, sel].mean(axis=0)
    xmean = xp.mean(axis=0)
    ...
    np.save(dest / 'test_targets.npy', yt)
    np.save(dest / 'test_predictions.npy', pred)
    np.save(dest / 'test_residuals.npy', (yt - pred).astype(np.float32))
    ...
    record = dict(encoder=enc, group='10x_block_A', train_samples=train, test_samples=test, n_train=len(y), n_test=len(yt), n_genes=len(genes), sklearn_version=sklearn.__version__, alpha=float(model.model.alpha), solver=model.model.solver, tol=float(model.model.tol), fit_intercept=True, pca_components=256, pca_variance_retained=float(model.pca.explained_variance_ratio_.sum()), seconds=time.monotonic() - started, independent_centered_target_max_prediction_difference=difference, stored_model_reproduction='exact', input_hashes=inputs)
...
def main():
    ...
    for enc in ['uni', 'virchow2', 'hoptimus0']:
        for k, sample in enumerate(SAMPLES[2:], start=2):
            fd = BASE / 'predictions' / enc / f'fold{k}'
            meta = {'test_samples': [sample], 'train_samples': [s for s in SAMPLES if s != sample]}
            assert meta['test_samples'] == [sample]
            assert set(meta['train_samples']) == set(SAMPLES) - {sample}
            registry.append(dict(encoder=enc, sample=sample, original_fold=k, prediction_dir=str(fd), train_samples=meta['train_samples'], test_samples=meta['test_samples'], predictions_sha256=digest(fd / 'test_predictions.npy')))
