...
def main():
    ...
    samples = cfg['all_samples'] + ['TENX111', 'TENX147', 'TENX148', 'TENX149', 'TENX13', 'TENX14', 'TENX39', 'TENX53', 'TENX68', 'NCBI776', 'NCBI681', 'NCBI682', 'NCBI683', 'NCBI684']
    ...
    with PdfPages(OUT / 'figures/patch_coverage_all_sections.pdf') as pdf:
        for sid in samples:
            ...
            retained = np.zeros(len(ids), dtype=bool)
            retained[ix] = True
            ...
            on_slide = (top_float[:, 0] + size_float >= 0) & (top_float[:, 0] < width) & (top_float[:, 1] + size_float >= 0) & (top_float[:, 1] < height)
            wholly_on_slide = (top[:, 0] >= 0) & (top[:, 1] >= 0) & (top[:, 0] + src <= width) & (top[:, 1] + src <= height)
            ...
            if masksrc:
                ...
                if invalid:
                    ...
                    issues.append({'sample': sid, 'issue': 'invalid_tissue_geometry_repaired_in_memory', 'count': invalid, 'area_change_px2': repair_area_delta, 'make_valid_vs_buffer_union_difference_px2': repair_union_difference})
            ...
            expected = on_slide & (area_frac >= 0.15)
            ...
            if mask_available and (false_included.any() or false_excluded.any()):
                issues.append({'sample': sid, 'issue': 'mask_rule_membership_difference', 'saved_but_rule_excluded': int(false_included.sum()), 'rule_included_but_no_patch': int(false_excluded.sum()), 'mask_origin': masksrc['origin']})
            frame = pd.DataFrame({'sample': sid, 'barcode': ids, 'x': xy[:, 0], 'y': xy[:, 1], 'has_patch': retained, 'tissue_fraction': area_frac, 'overlaps_slide': on_slide, 'fully_on_slide': wholly_on_slide, 'expected_patch_default_rule': expected, 'default_rule_available': mask_available})
            if counts is not None:
                panel = json.loads((PROJECT_ROOT / 'data/v3' / f'gene_list_{cohorts[sid]}.json').read_text())
                ...
                for group, mask in [('retained', retained), ('excluded', ~retained)]:
                    ...
                    for col in ['noncontrol_counts', 'panel_counts', 'panel_genes_detected', 'panel_sum_log1p', 'noncontrol_genes_detected']:
                        ...
                        for label, val in [('mean', np.mean(v)), ('median', np.median(v)), ('q25', np.quantile(v, 0.25)), ('q75', np.quantile(v, 0.75))]:
                            rec[col + '_' + label] = float(val)
                    distributions.append(rec)
                fractional = int(np.count_nonzero(counts != np.floor(counts)))
            else:
                fractional = None
            rec = {'sample': sid, 'cohort': cohorts.get(sid, 'coad' if sid in samples[18:22] else 'idc_visium'), 'raw_spots': len(ids), 'patches': len(pids), 'no_patch': int((~retained).sum()), 'max_patch_coordinate_difference_px': max_delta, 'patch_size_src': src, 'pixel_size_um': pixel, 'patch_footprint_um': src * pixel, 'nominal_output_pixel_um': pixel * float(at['downsample']), 'h5ad_display_spot_diameter_um': diam * pixel, 'mask_origin': masksrc['origin'] if masksrc else 'unavailable', 'metadata_origin': mdsrc['origin'] if mdsrc else 'unavailable', 'invalid_contour_count': invalid, 'geometry_repair_area_change_px2': repair_area_delta, 'make_valid_vs_buffer_union_difference_px2': repair_union_difference, 'mask_rule_available': bool(mask_available), 'mask_rule_exact_match': bool(mask_available and np.array_equal(expected, retained)), 'excluded_no_slide_overlap': int((~retained & ~on_slide).sum()), 'excluded_below_mask_threshold': int((~retained & on_slide & (area_frac < 0.15)).sum()), 'retained_below_mask_threshold': int((retained & (area_frac < 0.15)).sum()), 'retained_partially_outside_slide': int((retained & ~wholly_on_slide).sum()), 'saved_but_rule_excluded': int(false_included.sum()), 'rule_included_but_no_patch': int(false_excluded.sum()), 'fractional_count_entries': fractional}
            summary.append(rec)
            rows.append(frame)
            ...
            for ax in axes:
                ax.imshow(thumb)
                ax.set_axis_off()
            axes[0].scatter(xy[retained, 0] * thumb_scale, xy[retained, 1] * thumb_scale, c='#008080', s=1, alpha=0.3, label='Supplied patch')
            axes[0].scatter(xy[~retained, 0] * thumb_scale, xy[~retained, 1] * thumb_scale, c='#e66101', s=3, alpha=0.75, label='No patch')
            axes[0].legend(loc='upper right', fontsize=7)
            axes[0].set_title(f'{sid}: {len(pids):,}/{len(ids):,} locations retained')
            sc = axes[1].scatter(xy[:, 0] * thumb_scale, xy[:, 1] * thumb_scale, c=area_frac, cmap='viridis', vmin=0, vmax=1, s=2, alpha=0.6)
            fig.colorbar(sc, ax=axes[1], shrink=0.65, label='Patch fraction intersecting H&E mask')
            ...
            plt.close(fig)
            print(f"{sid}: coordinate delta={max_delta:g}; mask match={rec['mask_rule_exact_match']}; outside={rec['excluded_no_slide_overlap']}; low-mask={rec['excluded_below_mask_threshold']}; unexplained={int(false_excluded.sum())}", flush=True)
    ...
    (OUT / 'coverage_checks.json').write_text(json.dumps({'completed_utc': datetime.now(timezone.utc).isoformat(), 'n_sections': len(summary), 'coordinate_exact_sections': sum((r['max_patch_coordinate_difference_px'] == 0 for r in summary)), 'mask_exact_sections': sum((r['mask_rule_exact_match'] for r in summary)), 'issues': issues}, indent=2) + '\n')
